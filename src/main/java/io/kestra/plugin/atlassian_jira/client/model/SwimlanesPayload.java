...
import io.kestra.plugin.atlassian_jira.client.model.SwimlanePayload;
...
public class SwimlanesPayload {
  ...
  /**
   * The name of the custom swimlane to use for work items that don&#39;t match
   * any other swimlanes.
   * 
  ...
  static {
    ...
    openapiFields = new HashSet<String>(
        Arrays.asList("customSwimlanes", "defaultCustomSwimlaneName", "swimlaneStrategy"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!SwimlanesPayload.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in SwimlanesPayload is not found in the empty JSON string",
            SwimlanesPayload.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!SwimlanesPayload.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `SwimlanesPayload` properties. JSON: %s",
    ...
    if (jsonObj.get("customSwimlanes") != null && !jsonObj.get("customSwimlanes").isJsonNull()) {
      ...
      if (jsonArraycustomSwimlanes != null) {
        ...
        if (!jsonObj.get("customSwimlanes").isJsonArray()) {
          throw new IllegalArgumentException(String.format(Locale.ROOT,
              "Expected the field `customSwimlanes` to be an array in the JSON string but got `%s`",
              jsonObj.get("customSwimlanes").toString()));
        }
    ...
    if ((jsonObj.get("defaultCustomSwimlaneName") != null && !jsonObj.get("defaultCustomSwimlaneName").isJsonNull())
        && !jsonObj.get("defaultCustomSwimlaneName").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `defaultCustomSwimlaneName` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("defaultCustomSwimlaneName").toString()));
    }
    if ((jsonObj.get("swimlaneStrategy") != null && !jsonObj.get("swimlaneStrategy").isJsonNull())
        && !jsonObj.get("swimlaneStrategy").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `swimlaneStrategy` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("swimlaneStrategy").toString()));
    }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<SwimlanesPayload> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(SwimlanesPayload.class));
