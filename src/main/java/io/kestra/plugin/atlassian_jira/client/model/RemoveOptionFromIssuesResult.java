...
import io.kestra.plugin.atlassian_jira.client.model.SimpleErrorCollection;
...
public class RemoveOptionFromIssuesResult {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("errors", "modifiedIssues", "unmodifiedIssues"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!RemoveOptionFromIssuesResult.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is
                                                                           // null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in RemoveOptionFromIssuesResult is not found in the empty JSON string",
            RemoveOptionFromIssuesResult.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!RemoveOptionFromIssuesResult.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `RemoveOptionFromIssuesResult` properties. JSON: %s",
    ...
    if (jsonObj.get("modifiedIssues") != null && !jsonObj.get("modifiedIssues").isJsonNull()
        && !jsonObj.get("modifiedIssues").isJsonArray()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `modifiedIssues` to be an array in the JSON string but got `%s`",
          jsonObj.get("modifiedIssues").toString()));
    }
    // ensure the optional json data is an array if present
    if (jsonObj.get("unmodifiedIssues") != null && !jsonObj.get("unmodifiedIssues").isJsonNull()
        && !jsonObj.get("unmodifiedIssues").isJsonArray()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `unmodifiedIssues` to be an array in the JSON string but got `%s`",
          jsonObj.get("unmodifiedIssues").toString()));
    }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<RemoveOptionFromIssuesResult> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(RemoveOptionFromIssuesResult.class));
