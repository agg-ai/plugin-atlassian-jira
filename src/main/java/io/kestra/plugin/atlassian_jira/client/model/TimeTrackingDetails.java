...
public class TimeTrackingDetails {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("originalEstimate", "originalEstimateSeconds",
        "remainingEstimate", "remainingEstimateSeconds", "timeSpent", "timeSpentSeconds"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!TimeTrackingDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in TimeTrackingDetails is not found in the empty JSON string",
            TimeTrackingDetails.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!TimeTrackingDetails.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `TimeTrackingDetails` properties. JSON: %s",
            entry.getKey(), jsonElement.toString()));
      }
    }
    JsonObject jsonObj = jsonElement.getAsJsonObject();
    if ((jsonObj.get("originalEstimate") != null && !jsonObj.get("originalEstimate").isJsonNull())
        && !jsonObj.get("originalEstimate").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `originalEstimate` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("originalEstimate").toString()));
    }
    if ((jsonObj.get("remainingEstimate") != null && !jsonObj.get("remainingEstimate").isJsonNull())
        && !jsonObj.get("remainingEstimate").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `remainingEstimate` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("remainingEstimate").toString()));
    }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<TimeTrackingDetails> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(TimeTrackingDetails.class));
