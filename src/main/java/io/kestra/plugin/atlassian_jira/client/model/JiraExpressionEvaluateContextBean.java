...
import io.kestra.plugin.atlassian_jira.client.model.CustomContextVariable;
import io.kestra.plugin.atlassian_jira.client.model.IdOrKeyBean;
import io.kestra.plugin.atlassian_jira.client.model.JexpEvaluateCtxIssues;
...
public class JiraExpressionEvaluateContextBean {
   ...
   * for use in a custom context: * &#x60;user&#x60;: A
   ...
   * the same as the ID of the underlying Jira issue, but the customer request
  ...
  /**
   * The ID of the service desk that is available under the
  ...
  static {
    ...
    openapiFields = new HashSet<String>(
        Arrays.asList("board", "custom", "customerRequest", "issue", "issues", "project", "serviceDesk", "sprint"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!JiraExpressionEvaluateContextBean.openapiRequiredFields.isEmpty()) { // has required fields but JSON element
                                                                                // is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in JiraExpressionEvaluateContextBean is not found in the empty JSON string",
            JiraExpressionEvaluateContextBean.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!JiraExpressionEvaluateContextBean.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `JiraExpressionEvaluateContextBean` properties. JSON: %s",
      ...
      if (jsonArraycustom != null) {
        ...
        if (!jsonObj.get("custom").isJsonArray()) {
          throw new IllegalArgumentException(
              String.format(Locale.ROOT, "Expected the field `custom` to be an array in the JSON string but got `%s`",
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!JiraExpressionEvaluateContextBean.class.isAssignableFrom(type.getRawType())) {
        return null; // this class only serializes 'JiraExpressionEvaluateContextBean' and its
                     // subtypes
      }
      final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
      final TypeAdapter<JiraExpressionEvaluateContextBean> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(JiraExpressionEvaluateContextBean.class));
