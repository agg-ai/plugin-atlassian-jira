...
public class RequiredMappingByWorkflows {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!RequiredMappingByWorkflows.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in RequiredMappingByWorkflows is not found in the empty JSON string",
            RequiredMappingByWorkflows.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!RequiredMappingByWorkflows.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `RequiredMappingByWorkflows` properties. JSON: %s",
            entry.getKey(), jsonElement.toString()));
      }
    }
    JsonObject jsonObj = jsonElement.getAsJsonObject();
    if ((jsonObj.get("sourceWorkflowId") != null && !jsonObj.get("sourceWorkflowId").isJsonNull())
        && !jsonObj.get("sourceWorkflowId").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `sourceWorkflowId` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("sourceWorkflowId").toString()));
    }
    // ensure the optional json data is an array if present
    if (jsonObj.get("statusIds") != null && !jsonObj.get("statusIds").isJsonNull()
        && !jsonObj.get("statusIds").isJsonArray()) {
      throw new IllegalArgumentException(
          String.format(Locale.ROOT, "Expected the field `statusIds` to be an array in the JSON string but got `%s`",
              jsonObj.get("statusIds").toString()));
    }
    if ((jsonObj.get("targetWorkflowId") != null && !jsonObj.get("targetWorkflowId").isJsonNull())
        && !jsonObj.get("targetWorkflowId").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `targetWorkflowId` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("targetWorkflowId").toString()));
    }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<RequiredMappingByWorkflows> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(RequiredMappingByWorkflows.class));
