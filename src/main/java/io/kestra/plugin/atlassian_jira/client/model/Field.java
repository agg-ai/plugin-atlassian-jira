...
import io.kestra.plugin.atlassian_jira.client.model.FieldLastUsed;
import io.kestra.plugin.atlassian_jira.client.model.JsonTypeBean;
...
public class Field {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(contextsCount, description, id, isLocked, isUnscreenable, key, lastUsed, name, projectsCount,
        schema, screensCount, searcherKey, stableId, typeDisplayName);
  }
  ...
  static {
    ...
    openapiFields = new HashSet<String>(
        Arrays.asList("contextsCount", "description", "id", "isLocked", "isUnscreenable", "key", "lastUsed", "name",
            "projectsCount", "schema", "screensCount", "searcherKey", "stableId", "typeDisplayName"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!Field.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(
            String.format(Locale.ROOT, "The required field(s) %s in Field is not found in the empty JSON string",
                Field.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!Field.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `Field` properties. JSON: %s", entry.getKey(),
    ...
    if ((jsonObj.get("stableId") != null && !jsonObj.get("stableId").isJsonNull())
        && !jsonObj.get("stableId").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `stableId` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("stableId").toString()));
    }
    if ((jsonObj.get("typeDisplayName") != null && !jsonObj.get("typeDisplayName").isJsonNull())
        && !jsonObj.get("typeDisplayName").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `typeDisplayName` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("typeDisplayName").toString()));
    }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<Field> thisAdapter = gson.getDelegateAdapter(this, TypeToken.get(Field.class));
