...
import io.kestra.plugin.atlassian_jira.client.model.IssuesMetaBean;
...
public class JiraExpressionEvaluationMetaDataBean {
  ...
  public JiraExpressionEvaluationMetaDataBean complexity(
      @javax.annotation.Nullable JiraExpressionsComplexityBean complexity) {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!JiraExpressionEvaluationMetaDataBean.openapiRequiredFields.isEmpty()) { // has required fields but JSON
                                                                                   // element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in JiraExpressionEvaluationMetaDataBean is not found in the empty JSON string",
            JiraExpressionEvaluationMetaDataBean.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!JiraExpressionEvaluationMetaDataBean.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `JiraExpressionEvaluationMetaDataBean` properties. JSON: %s",
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!JiraExpressionEvaluationMetaDataBean.class.isAssignableFrom(type.getRawType())) {
        return null; // this class only serializes 'JiraExpressionEvaluationMetaDataBean' and its
                     // subtypes
      }
      final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
      final TypeAdapter<JiraExpressionEvaluationMetaDataBean> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(JiraExpressionEvaluationMetaDataBean.class));
  ...
  /**
   * Create an instance of JiraExpressionEvaluationMetaDataBean given an JSON
   * string
   *
