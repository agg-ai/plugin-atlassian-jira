...
public class CustomFieldContextDefaultValueDateTime {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!CustomFieldContextDefaultValueDateTime.openapiRequiredFields.isEmpty()) { // has required fields but JSON
                                                                                     // element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in CustomFieldContextDefaultValueDateTime is not found in the empty JSON string",
            CustomFieldContextDefaultValueDateTime.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!CustomFieldContextDefaultValueDateTime.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `CustomFieldContextDefaultValueDateTime` properties. JSON: %s",
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!CustomFieldContextDefaultValueDateTime.class.isAssignableFrom(type.getRawType())) {
        return null; // this class only serializes 'CustomFieldContextDefaultValueDateTime' and its
                     // subtypes
      }
      final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
      final TypeAdapter<CustomFieldContextDefaultValueDateTime> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(CustomFieldContextDefaultValueDateTime.class));
  ...
  /**
   * Create an instance of CustomFieldContextDefaultValueDateTime given an JSON
   * string
   *
  ...
  /**
   * Convert an instance of CustomFieldContextDefaultValueDateTime to an JSON
   * string
   *
   * @return JSON string
   */
