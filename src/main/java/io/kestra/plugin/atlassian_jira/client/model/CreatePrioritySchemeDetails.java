...
public class CreatePrioritySchemeDetails {
   ...
   * **priorities** are **added** to the new priority scheme, no mapping needs to
   * be provided as the new priorities are not used by any issues. * When
   * **priorities** are **removed** from the new priority scheme, no mapping needs
   * to be provided as the removed priorities are not used by any issues. * When
   * **projects** are **added** to the priority scheme, the priorities of issues
   * in those projects might need to be migrated to new priorities used by the
   * priority scheme. This can occur when the current scheme does not use all the
   * priorities in the project(s)&#39; priority scheme(s). * An &#x60;in&#x60;
   * mapping must be provided for each of these priorities. * When **projects**
   * are **removed** from the priority scheme, no mapping needs to be provided as
   * the removed projects are not using the priorities of the new priority scheme.
   * For more information on &#x60;in&#x60; and &#x60;out&#x60; mappings, see the
   * child properties documentation for the &#x60;PriorityMapping&#x60; object
   * below.
   * 
   * @return mappings
   */
  ...
  static {
    ...
    openapiFields = new HashSet<String>(
        Arrays.asList("defaultPriorityId", "description", "mappings", "name", "priorityIds", "projectIds"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("defaultPriorityId", "name", "priorityIds"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!CreatePrioritySchemeDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is
                                                                          // null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in CreatePrioritySchemeDetails is not found in the empty JSON string",
            CreatePrioritySchemeDetails.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!CreatePrioritySchemeDetails.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `CreatePrioritySchemeDetails` properties. JSON: %s",
    ...
    if (jsonObj.get("priorityIds") == null) {
      ...
      throw new IllegalArgumentException(
          String.format(Locale.ROOT, "Expected the field `priorityIds` to be an array in the JSON string but got `%s`",
              jsonObj.get("priorityIds").toString()));
    }
    // ensure the optional json data is an array if present
    if (jsonObj.get("projectIds") != null && !jsonObj.get("projectIds").isJsonNull()
        && !jsonObj.get("projectIds").isJsonArray()) {
      throw new IllegalArgumentException(
          String.format(Locale.ROOT, "Expected the field `projectIds` to be an array in the JSON string but got `%s`",
              jsonObj.get("projectIds").toString()));
    }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<CreatePrioritySchemeDetails> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(CreatePrioritySchemeDetails.class));
