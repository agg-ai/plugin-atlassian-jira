...
import io.kestra.plugin.atlassian_jira.client.model.PriorityMapping;
import io.kestra.plugin.atlassian_jira.client.model.UpdatePrioritiesInSchemeRequestBean;
import io.kestra.plugin.atlassian_jira.client.model.UpdateProjectsInSchemeRequestBean;
...
public class UpdatePrioritySchemeRequestBean {
  ...
  /**
   * Instructions to migrate the priorities of issues. &#x60;in&#x60; mappings are
   * used to migrate the priorities of issues to priorities used within the
   * priority scheme. &#x60;out&#x60; mappings are used to migrate the priorities
   * of issues to priorities not used within the priority scheme. * When
   * **priorities** are **added** to the priority scheme, no mapping needs to be
   * provided as the new priorities are not used by any issues. * When
   * **priorities** are **removed** from the priority scheme, issues that are
   * using those priorities must be migrated to new priorities used by the
   * priority scheme. * An &#x60;in&#x60; mapping must be provided for each of
   * these priorities. * When **projects** are **added** to the priority scheme,
   * the priorities of issues in those projects might need to be migrated to new
   * priorities used by the priority scheme. This can occur when the current
   * scheme does not use all the priorities in the project(s)&#39; priority
   * scheme(s). * An &#x60;in&#x60; mapping must be provided for each of these
   * priorities. * When **projects** are **removed** from the priority scheme, the
   * priorities of issues in those projects might need to be migrated to new
   * priorities within the **Default Priority Scheme** that are not used by the
   * priority scheme. This can occur when the **Default Priority Scheme** does not
   * use all the priorities within the current scheme. * An &#x60;out&#x60;
   * mapping must be provided for each of these priorities. For more information
   * on &#x60;in&#x60; and &#x60;out&#x60; mappings, see the child properties
  ...
  public UpdatePrioritySchemeRequestBean priorities(
      @javax.annotation.Nullable UpdatePrioritiesInSchemeRequestBean priorities) {
  ...
  public UpdatePrioritySchemeRequestBean projects(
      @javax.annotation.Nullable UpdateProjectsInSchemeRequestBean projects) {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(
        Arrays.asList("defaultPriorityId", "description", "mappings", "name", "priorities", "projects"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!UpdatePrioritySchemeRequestBean.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is
                                                                              // null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in UpdatePrioritySchemeRequestBean is not found in the empty JSON string",
            UpdatePrioritySchemeRequestBean.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!UpdatePrioritySchemeRequestBean.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `UpdatePrioritySchemeRequestBean` properties. JSON: %s",
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<UpdatePrioritySchemeRequestBean> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(UpdatePrioritySchemeRequestBean.class));
