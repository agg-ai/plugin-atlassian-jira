...
public class UiModificationContextDetails {
   ...
   * &#x60;IssueView&#x60; and &#x60;IssueTransition&#x60; are supported. Null is
   * treated as a wildcard, meaning the UI modification will be applied to all
   * view types. Each UI modification context can have a maximum of one wildcard.
   */
  ...
  /**
   * The issue type ID of the context. Null is treated as a wildcard, meaning the
   * UI modification will be applied to all issue types. Each UI modification
   ...
   * modification will be applied to all projects. Each UI modification context
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!UiModificationContextDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is
                                                                           // null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in UiModificationContextDetails is not found in the empty JSON string",
            UiModificationContextDetails.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!UiModificationContextDetails.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `UiModificationContextDetails` properties. JSON: %s",
    ...
    if ((jsonObj.get("issueTypeId") != null && !jsonObj.get("issueTypeId").isJsonNull())
        && !jsonObj.get("issueTypeId").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
    ...
    if ((jsonObj.get("viewType") != null && !jsonObj.get("viewType").isJsonNull())
        && !jsonObj.get("viewType").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `viewType` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("viewType").toString()));
    }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<UiModificationContextDetails> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(UiModificationContextDetails.class));
