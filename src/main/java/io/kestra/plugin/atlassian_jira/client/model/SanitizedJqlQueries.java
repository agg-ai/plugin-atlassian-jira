...
import io.kestra.plugin.atlassian_jira.client.model.SanitizedJqlQuery;
...
public class SanitizedJqlQueries {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!SanitizedJqlQueries.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in SanitizedJqlQueries is not found in the empty JSON string",
            SanitizedJqlQueries.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!SanitizedJqlQueries.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `SanitizedJqlQueries` properties. JSON: %s",
      ...
      if (jsonArrayqueries != null) {
        ...
        if (!jsonObj.get("queries").isJsonArray()) {
          throw new IllegalArgumentException(
              String.format(Locale.ROOT, "Expected the field `queries` to be an array in the JSON string but got `%s`",
                  jsonObj.get("queries").toString()));
        }

        // validate the optional field `queries` (array)
        for (int i = 0; i < jsonArrayqueries.size(); i++) {
          SanitizedJqlQuery.validateJsonElement(jsonArrayqueries.get(i));
        }
        ;
      }
    }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<SanitizedJqlQueries> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(SanitizedJqlQueries.class));
