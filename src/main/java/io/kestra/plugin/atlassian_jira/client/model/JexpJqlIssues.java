 ...
 * under the &#x60;issues&#x60; context variable. Not all issues returned by the
 * JQL query are loaded, only those described by the &#x60;startAt&#x60; and
 * &#x60;maxResults&#x60; properties. To determine whether it is necessary to
 * iterate to ensure all the issues returned by the JQL query are evaluated,
...
public class JexpJqlIssues {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!JexpJqlIssues.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in JexpJqlIssues is not found in the empty JSON string",
            JexpJqlIssues.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!JexpJqlIssues.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `JexpJqlIssues` properties. JSON: %s",
    ...
    if ((jsonObj.get("validation") != null && !jsonObj.get("validation").isJsonNull())
        && !jsonObj.get("validation").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `validation` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("validation").toString()));
    }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<JexpJqlIssues> thisAdapter = gson.getDelegateAdapter(this, TypeToken.get(JexpJqlIssues.class));
