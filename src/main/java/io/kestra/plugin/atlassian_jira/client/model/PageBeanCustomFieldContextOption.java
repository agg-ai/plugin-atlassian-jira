...
public class PageBeanCustomFieldContextOption {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!PageBeanCustomFieldContextOption.openapiRequiredFields.isEmpty()) { // has required fields but JSON element
                                                                               // is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in PageBeanCustomFieldContextOption is not found in the empty JSON string",
            PageBeanCustomFieldContextOption.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!PageBeanCustomFieldContextOption.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `PageBeanCustomFieldContextOption` properties. JSON: %s",
    ...
    if (jsonObj.get("values") != null && !jsonObj.get("values").isJsonNull()) {
      ...
      if (jsonArrayvalues != null) {
        ...
        for (int i = 0; i < jsonArrayvalues.size(); i++) {
          CustomFieldContextOption.validateJsonElement(jsonArrayvalues.get(i));
        }
        ;
      }
    }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!PageBeanCustomFieldContextOption.class.isAssignableFrom(type.getRawType())) {
        return null; // this class only serializes 'PageBeanCustomFieldContextOption' and its
                     // subtypes
      }
      final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
      final TypeAdapter<PageBeanCustomFieldContextOption> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(PageBeanCustomFieldContextOption.class));
