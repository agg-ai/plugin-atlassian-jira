...
public class JiraLabelPropertiesInputJackson1 {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!JiraLabelPropertiesInputJackson1.openapiRequiredFields.isEmpty()) { // has required fields but JSON element
                                                                               // is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in JiraLabelPropertiesInputJackson1 is not found in the empty JSON string",
            JiraLabelPropertiesInputJackson1.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!JiraLabelPropertiesInputJackson1.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `JiraLabelPropertiesInputJackson1` properties. JSON: %s",
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!JiraLabelPropertiesInputJackson1.class.isAssignableFrom(type.getRawType())) {
        return null; // this class only serializes 'JiraLabelPropertiesInputJackson1' and its
                     // subtypes
      }
      final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
      final TypeAdapter<JiraLabelPropertiesInputJackson1> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(JiraLabelPropertiesInputJackson1.class));
