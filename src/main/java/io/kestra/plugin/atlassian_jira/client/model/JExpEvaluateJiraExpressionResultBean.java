...
import io.kestra.plugin.atlassian_jira.client.model.JExpEvaluateMetaDataBean;
...
public class JExpEvaluateJiraExpressionResultBean {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!JExpEvaluateJiraExpressionResultBean.openapiRequiredFields.isEmpty()) { // has required fields but JSON
                                                                                   // element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in JExpEvaluateJiraExpressionResultBean is not found in the empty JSON string",
            JExpEvaluateJiraExpressionResultBean.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!JExpEvaluateJiraExpressionResultBean.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `JExpEvaluateJiraExpressionResultBean` properties. JSON: %s",
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!JExpEvaluateJiraExpressionResultBean.class.isAssignableFrom(type.getRawType())) {
        return null; // this class only serializes 'JExpEvaluateJiraExpressionResultBean' and its
                     // subtypes
      }
      final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
      final TypeAdapter<JExpEvaluateJiraExpressionResultBean> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(JExpEvaluateJiraExpressionResultBean.class));
  ...
  /**
   * Create an instance of JExpEvaluateJiraExpressionResultBean given an JSON
   * string
   *
