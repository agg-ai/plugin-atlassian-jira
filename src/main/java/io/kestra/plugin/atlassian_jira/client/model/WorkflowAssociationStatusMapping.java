...
public class WorkflowAssociationStatusMapping {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!WorkflowAssociationStatusMapping.openapiRequiredFields.isEmpty()) { // has required fields but JSON element
                                                                               // is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in WorkflowAssociationStatusMapping is not found in the empty JSON string",
            WorkflowAssociationStatusMapping.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!WorkflowAssociationStatusMapping.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `WorkflowAssociationStatusMapping` properties. JSON: %s",
    ...
    if (!jsonObj.get("newStatusId").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `newStatusId` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("newStatusId").toString()));
    }
    if (!jsonObj.get("oldStatusId").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `oldStatusId` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("oldStatusId").toString()));
    }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!WorkflowAssociationStatusMapping.class.isAssignableFrom(type.getRawType())) {
        return null; // this class only serializes 'WorkflowAssociationStatusMapping' and its
                     // subtypes
      }
      final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
      final TypeAdapter<WorkflowAssociationStatusMapping> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(WorkflowAssociationStatusMapping.class));
