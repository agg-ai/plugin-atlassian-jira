...
import io.kestra.plugin.atlassian_jira.client.model.ProjectWithDataPolicy;
...
public class ProjectDataPolicies {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!ProjectDataPolicies.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in ProjectDataPolicies is not found in the empty JSON string",
            ProjectDataPolicies.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!ProjectDataPolicies.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `ProjectDataPolicies` properties. JSON: %s",
    ...
    if (jsonObj.get("projectDataPolicies") != null && !jsonObj.get("projectDataPolicies").isJsonNull()) {
      ...
      if (jsonArrayprojectDataPolicies != null) {
        ...
        if (!jsonObj.get("projectDataPolicies").isJsonArray()) {
          throw new IllegalArgumentException(String.format(Locale.ROOT,
              "Expected the field `projectDataPolicies` to be an array in the JSON string but got `%s`",
              jsonObj.get("projectDataPolicies").toString()));
        }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<ProjectDataPolicies> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(ProjectDataPolicies.class));
