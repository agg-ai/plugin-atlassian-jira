...
/**
 * Details of changes to a priority scheme&#39;s priorities that require
...
public class SuggestedMappingsForPrioritiesRequestBean {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!SuggestedMappingsForPrioritiesRequestBean.openapiRequiredFields.isEmpty()) { // has required fields but JSON
                                                                                        // element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in SuggestedMappingsForPrioritiesRequestBean is not found in the empty JSON string",
            SuggestedMappingsForPrioritiesRequestBean.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!SuggestedMappingsForPrioritiesRequestBean.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `SuggestedMappingsForPrioritiesRequestBean` properties. JSON: %s",
    ...
    if (jsonObj.get("add") != null && !jsonObj.get("add").isJsonNull() && !jsonObj.get("add").isJsonArray()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `add` to be an array in the JSON string but got `%s`", jsonObj.get("add").toString()));
    }
    // ensure the optional json data is an array if present
    if (jsonObj.get("remove") != null && !jsonObj.get("remove").isJsonNull() && !jsonObj.get("remove").isJsonArray()) {
      throw new IllegalArgumentException(
          String.format(Locale.ROOT, "Expected the field `remove` to be an array in the JSON string but got `%s`",
              jsonObj.get("remove").toString()));
    }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!SuggestedMappingsForPrioritiesRequestBean.class.isAssignableFrom(type.getRawType())) {
        return null; // this class only serializes 'SuggestedMappingsForPrioritiesRequestBean' and
                     // its subtypes
      }
      final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
      final TypeAdapter<SuggestedMappingsForPrioritiesRequestBean> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(SuggestedMappingsForPrioritiesRequestBean.class));
  ...
  /**
   * Create an instance of SuggestedMappingsForPrioritiesRequestBean given an JSON
   * string
   *
  ...
  /**
   * Convert an instance of SuggestedMappingsForPrioritiesRequestBean to an JSON
   * string
   *
   * @return JSON string
   */
