...
public class SanitizedJqlQuery {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!SanitizedJqlQuery.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in SanitizedJqlQuery is not found in the empty JSON string",
            SanitizedJqlQuery.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!SanitizedJqlQuery.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `SanitizedJqlQuery` properties. JSON: %s",
    ...
    if ((jsonObj.get("initialQuery") != null && !jsonObj.get("initialQuery").isJsonNull())
        && !jsonObj.get("initialQuery").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `initialQuery` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("initialQuery").toString()));
    }
    if ((jsonObj.get("sanitizedQuery") != null && !jsonObj.get("sanitizedQuery").isJsonNull())
        && !jsonObj.get("sanitizedQuery").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `sanitizedQuery` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("sanitizedQuery").toString()));
    }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<SanitizedJqlQuery> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(SanitizedJqlQuery.class));
