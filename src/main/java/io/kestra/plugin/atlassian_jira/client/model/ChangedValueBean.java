...
public class ChangedValueBean {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!ChangedValueBean.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in ChangedValueBean is not found in the empty JSON string",
            ChangedValueBean.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!ChangedValueBean.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `ChangedValueBean` properties. JSON: %s",
            entry.getKey(), jsonElement.toString()));
      }
    }
    JsonObject jsonObj = jsonElement.getAsJsonObject();
    if ((jsonObj.get("changedFrom") != null && !jsonObj.get("changedFrom").isJsonNull())
        && !jsonObj.get("changedFrom").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `changedFrom` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("changedFrom").toString()));
    }
    if ((jsonObj.get("changedTo") != null && !jsonObj.get("changedTo").isJsonNull())
        && !jsonObj.get("changedTo").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `changedTo` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("changedTo").toString()));
    }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<ChangedValueBean> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(ChangedValueBean.class));
