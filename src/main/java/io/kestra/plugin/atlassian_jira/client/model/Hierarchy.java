...
import io.kestra.plugin.atlassian_jira.client.model.SimplifiedHierarchyLevel;
...
public class Hierarchy {
    ...
    if (jsonElement == null) {
      if (!Hierarchy.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(
            String.format(Locale.ROOT, "The required field(s) %s in Hierarchy is not found in the empty JSON string",
                Hierarchy.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!Hierarchy.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `Hierarchy` properties. JSON: %s", entry.getKey(),
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<Hierarchy> thisAdapter = gson.getDelegateAdapter(this, TypeToken.get(Hierarchy.class));
