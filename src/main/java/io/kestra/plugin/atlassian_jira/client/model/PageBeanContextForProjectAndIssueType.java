...
import io.kestra.plugin.atlassian_jira.client.model.ContextForProjectAndIssueType;
...
public class PageBeanContextForProjectAndIssueType {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!PageBeanContextForProjectAndIssueType.openapiRequiredFields.isEmpty()) { // has required fields but JSON
                                                                                    // element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in PageBeanContextForProjectAndIssueType is not found in the empty JSON string",
            PageBeanContextForProjectAndIssueType.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!PageBeanContextForProjectAndIssueType.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `PageBeanContextForProjectAndIssueType` properties. JSON: %s",
    ...
    if (jsonObj.get("values") != null && !jsonObj.get("values").isJsonNull()) {
      ...
      if (jsonArrayvalues != null) {
        ...
        for (int i = 0; i < jsonArrayvalues.size(); i++) {
          ContextForProjectAndIssueType.validateJsonElement(jsonArrayvalues.get(i));
        }
        ;
      }
    }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!PageBeanContextForProjectAndIssueType.class.isAssignableFrom(type.getRawType())) {
        return null; // this class only serializes 'PageBeanContextForProjectAndIssueType' and its
                     // subtypes
      }
      final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
      final TypeAdapter<PageBeanContextForProjectAndIssueType> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(PageBeanContextForProjectAndIssueType.class));
  ...
  /**
   * Create an instance of PageBeanContextForProjectAndIssueType given an JSON
   * string
   *
  ...
  /**
   * Convert an instance of PageBeanContextForProjectAndIssueType to an JSON
   * string
   *
   * @return JSON string
   */
