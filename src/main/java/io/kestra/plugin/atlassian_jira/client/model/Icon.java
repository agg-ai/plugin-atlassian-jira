...
/**
 * An icon. If no icon is defined: * for a status icon, no status icon displays
 * in Jira. * for the remote object icon, the default link icon displays in
 * Jira.
 */
...
public class Icon {
  ...
  /**
   * The title of the icon. This is used as follows: * For a status icon it is
   * used as a tooltip on the icon. If not set, the status icon doesn&#39;t
   * display a tooltip in Jira. * For the remote object icon it is used in
   * conjunction with the application name to display a tooltip for the link&#39;s
   * icon. The tooltip takes the format \&quot;\\[application name\\] icon
   * title\&quot;. Blank itemsare excluded from the tooltip title. If both items
   * are blank, the icon tooltop displays as \&quot;Web Link\&quot;.
   * 
   * @return title
   */
  @javax.annotation.Nullable
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!Icon.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(
            String.format(Locale.ROOT, "The required field(s) %s in Icon is not found in the empty JSON string",
                Icon.openapiRequiredFields.toString()));
      }
    }
    JsonObject jsonObj = jsonElement.getAsJsonObject();
    if ((jsonObj.get("link") != null && !jsonObj.get("link").isJsonNull()) && !jsonObj.get("link").isJsonPrimitive()) {
      throw new IllegalArgumentException(
          String.format(Locale.ROOT, "Expected the field `link` to be a primitive type in the JSON string but got `%s`",
    ...
    if ((jsonObj.get("url16x16") != null && !jsonObj.get("url16x16").isJsonNull())
        && !jsonObj.get("url16x16").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `url16x16` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("url16x16").toString()));
    }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<Icon> thisAdapter = gson.getDelegateAdapter(this, TypeToken.get(Icon.class));
