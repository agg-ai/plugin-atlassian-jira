 ...
 * &#x60;(labels in (urgent, blocker) OR lastCommentedBy &#x3D; currentUser()).
 * Note that, where nesting is not defined, the parser nests JQL clauses based
 * on the operator precedence. For example, \&quot;A OR B AND C\&quot; is parsed
 * as \&quot;(A OR B) AND C\&quot;. See Setting the precedence of operators for
...
public class CompoundClause {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("clauses", "operator"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("clauses", "operator"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!CompoundClause.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in CompoundClause is not found in the empty JSON string",
            CompoundClause.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!CompoundClause.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `CompoundClause` properties. JSON: %s",
    ...
    if (!jsonObj.get("clauses").isJsonArray()) {
      throw new IllegalArgumentException(
          String.format(Locale.ROOT, "Expected the field `clauses` to be an array in the JSON string but got `%s`",
              jsonObj.get("clauses").toString()));
    }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<CompoundClause> thisAdapter = gson.getDelegateAdapter(this,
