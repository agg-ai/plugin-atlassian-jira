...
public class ActorInputBean {
   ...
   * used with the &#x60;groupId&#x60; parameter. As a group&#39;s name can
   ...
   * with the &#x60;group&#x60; parameter This parameter accepts a comma-separated
   * list. For example,
  ...
  /**
   * The account IDs of the users to add as default actors. This parameter accepts
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!ActorInputBean.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in ActorInputBean is not found in the empty JSON string",
            ActorInputBean.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!ActorInputBean.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `ActorInputBean` properties. JSON: %s",
    ...
    if (jsonObj.get("group") != null && !jsonObj.get("group").isJsonNull() && !jsonObj.get("group").isJsonArray()) {
      throw new IllegalArgumentException(
          String.format(Locale.ROOT, "Expected the field `group` to be an array in the JSON string but got `%s`",
              jsonObj.get("group").toString()));
    }
    // ensure the optional json data is an array if present
    if (jsonObj.get("groupId") != null && !jsonObj.get("groupId").isJsonNull()
        && !jsonObj.get("groupId").isJsonArray()) {
      throw new IllegalArgumentException(
          String.format(Locale.ROOT, "Expected the field `groupId` to be an array in the JSON string but got `%s`",
              jsonObj.get("groupId").toString()));
    }
    // ensure the optional json data is an array if present
    if (jsonObj.get("user") != null && !jsonObj.get("user").isJsonNull() && !jsonObj.get("user").isJsonArray()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `user` to be an array in the JSON string but got `%s`", jsonObj.get("user").toString()));
    }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<ActorInputBean> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(ActorInputBean.class));
