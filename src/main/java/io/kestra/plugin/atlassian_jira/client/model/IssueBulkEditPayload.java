...
import io.kestra.plugin.atlassian_jira.client.model.JiraIssueFields;
...
public class IssueBulkEditPayload {
   ...
   * of field being edited. Although the order is not significant, ensure that
   * field IDs align with those in selectedActions.
   * 
   ...
   * list corresponds to a specific attribute of an issue that is set to be
   * modified in the bulk edit operation. The relevant field ID can be obtained by
   * calling the Bulk Edit Get Fields REST API (documentation available on this
   * page itself).
   * 
  ...
  /**
   * List of issue IDs or keys which are to be bulk edited. These IDs or keys can
   ...
   * when the issues are being edited. If &#x60;true&#x60;, dispatches a bulk
  ...
  static {
    ...
    openapiFields = new HashSet<String>(
        Arrays.asList("editedFieldsInput", "selectedActions", "selectedIssueIdsOrKeys", "sendBulkNotification"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(
        Arrays.asList("editedFieldsInput", "selectedActions", "selectedIssueIdsOrKeys"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!IssueBulkEditPayload.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in IssueBulkEditPayload is not found in the empty JSON string",
            IssueBulkEditPayload.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!IssueBulkEditPayload.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `IssueBulkEditPayload` properties. JSON: %s",
    ...
    if (jsonObj.get("selectedActions") == null) {
      ...
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `selectedActions` to be an array in the JSON string but got `%s`",
          jsonObj.get("selectedActions").toString()));
    }
    // ensure the required json array is present
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<IssueBulkEditPayload> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(IssueBulkEditPayload.class));
