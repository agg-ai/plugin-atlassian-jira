...
public class CustomFieldContextDefaultValueForgeMultiStringField {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("type", "values"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!CustomFieldContextDefaultValueForgeMultiStringField.openapiRequiredFields.isEmpty()) { // has required fields
                                                                                                  // but JSON element is
                                                                                                  // null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in CustomFieldContextDefaultValueForgeMultiStringField is not found in the empty JSON string",
            CustomFieldContextDefaultValueForgeMultiStringField.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!CustomFieldContextDefaultValueForgeMultiStringField.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `CustomFieldContextDefaultValueForgeMultiStringField` properties. JSON: %s",
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!CustomFieldContextDefaultValueForgeMultiStringField.class.isAssignableFrom(type.getRawType())) {
        return null; // this class only serializes
                     // 'CustomFieldContextDefaultValueForgeMultiStringField' and its subtypes
      }
      final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
      final TypeAdapter<CustomFieldContextDefaultValueForgeMultiStringField> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(CustomFieldContextDefaultValueForgeMultiStringField.class));

      return (TypeAdapter<T>) new TypeAdapter<CustomFieldContextDefaultValueForgeMultiStringField>() {
        @Override
        public void write(JsonWriter out, CustomFieldContextDefaultValueForgeMultiStringField value)
            throws IOException {
  ...
  /**
   * Create an instance of CustomFieldContextDefaultValueForgeMultiStringField
  ...
  /**
   * Convert an instance of CustomFieldContextDefaultValueForgeMultiStringField to
   * an JSON string
   *
   * @return JSON string
   */
