...
import io.kestra.plugin.atlassian_jira.client.model.MultipleCustomFieldValuesUpdate;
...
public class MultipleCustomFieldValuesUpdateDetails {
  ...
  public MultipleCustomFieldValuesUpdateDetails updates(
      @javax.annotation.Nullable List<MultipleCustomFieldValuesUpdate> updates) {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!MultipleCustomFieldValuesUpdateDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON
                                                                                     // element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in MultipleCustomFieldValuesUpdateDetails is not found in the empty JSON string",
            MultipleCustomFieldValuesUpdateDetails.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!MultipleCustomFieldValuesUpdateDetails.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `MultipleCustomFieldValuesUpdateDetails` properties. JSON: %s",
      ...
      if (jsonArrayupdates != null) {
        ...
        if (!jsonObj.get("updates").isJsonArray()) {
          throw new IllegalArgumentException(
              String.format(Locale.ROOT, "Expected the field `updates` to be an array in the JSON string but got `%s`",
                  jsonObj.get("updates").toString()));
        }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!MultipleCustomFieldValuesUpdateDetails.class.isAssignableFrom(type.getRawType())) {
        return null; // this class only serializes 'MultipleCustomFieldValuesUpdateDetails' and its
                     // subtypes
      }
      final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
      final TypeAdapter<MultipleCustomFieldValuesUpdateDetails> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(MultipleCustomFieldValuesUpdateDetails.class));
  ...
  /**
   * Create an instance of MultipleCustomFieldValuesUpdateDetails given an JSON
   * string
   *
  ...
  /**
   * Convert an instance of MultipleCustomFieldValuesUpdateDetails to an JSON
   * string
   *
   * @return JSON string
   */
