...
public class JiraTimeTrackingField {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!JiraTimeTrackingField.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in JiraTimeTrackingField is not found in the empty JSON string",
            JiraTimeTrackingField.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!JiraTimeTrackingField.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `JiraTimeTrackingField` properties. JSON: %s",
    ...
    if (!jsonObj.get("timeRemaining").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `timeRemaining` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("timeRemaining").toString()));
    }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<JiraTimeTrackingField> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(JiraTimeTrackingField.class));
