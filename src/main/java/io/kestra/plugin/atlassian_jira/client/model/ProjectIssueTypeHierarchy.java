...
import io.kestra.plugin.atlassian_jira.client.model.ProjectIssueTypesHierarchyLevel;
...
public class ProjectIssueTypeHierarchy {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!ProjectIssueTypeHierarchy.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in ProjectIssueTypeHierarchy is not found in the empty JSON string",
            ProjectIssueTypeHierarchy.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!ProjectIssueTypeHierarchy.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `ProjectIssueTypeHierarchy` properties. JSON: %s",
    ...
    if (jsonObj.get("hierarchy") != null && !jsonObj.get("hierarchy").isJsonNull()) {
      ...
      if (jsonArrayhierarchy != null) {
        ...
        if (!jsonObj.get("hierarchy").isJsonArray()) {
          throw new IllegalArgumentException(String.format(Locale.ROOT,
              "Expected the field `hierarchy` to be an array in the JSON string but got `%s`",
              jsonObj.get("hierarchy").toString()));
        }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<ProjectIssueTypeHierarchy> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(ProjectIssueTypeHierarchy.class));
