...
public class JiraExpressionComplexity {
   ...
   * formula or number. For example: * &#x60;issues.map(i &#x3D;&gt;
   * i.comments)&#x60; performs as many expensive operations as there are issues
   * on the issues list. So this parameter returns &#x60;N&#x60;, where
   * &#x60;N&#x60; is the size of issue list. * &#x60;new
   * Issue(10010).comments&#x60; gets comments for one issue, so its complexity is
   * &#x60;2&#x60; (&#x60;1&#x60; to retrieve issue 10010 from the database plus
   * &#x60;1&#x60; to get its comments).
   * 
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("expensiveOperations", "variables"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("expensiveOperations"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!JiraExpressionComplexity.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in JiraExpressionComplexity is not found in the empty JSON string",
            JiraExpressionComplexity.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!JiraExpressionComplexity.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `JiraExpressionComplexity` properties. JSON: %s",
    ...
    if (!jsonObj.get("expensiveOperations").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `expensiveOperations` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("expensiveOperations").toString()));
    }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<JiraExpressionComplexity> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(JiraExpressionComplexity.class));
