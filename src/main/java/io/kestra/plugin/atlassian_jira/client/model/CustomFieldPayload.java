...
public class CustomFieldPayload {
  ...
  /**
   * Allows an overwrite to declare the new Custom Field to be created as a
   * GLOBAL-scoped field. Leave this as empty or null to use the project&#39;s
   * default scope.
   */
  ...
  static {
    ...
    openapiFields = new HashSet<String>(
        Arrays.asList("cfType", "description", "name", "onConflict", "pcri", "scope", "searcherKey"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!CustomFieldPayload.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in CustomFieldPayload is not found in the empty JSON string",
            CustomFieldPayload.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!CustomFieldPayload.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `CustomFieldPayload` properties. JSON: %s",
            entry.getKey(), jsonElement.toString()));
      }
    }
    JsonObject jsonObj = jsonElement.getAsJsonObject();
    if ((jsonObj.get("cfType") != null && !jsonObj.get("cfType").isJsonNull())
        && !jsonObj.get("cfType").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `cfType` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("cfType").toString()));
    }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<CustomFieldPayload> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(CustomFieldPayload.class));
