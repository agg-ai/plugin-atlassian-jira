...
public class VersionRelatedWork {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!VersionRelatedWork.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in VersionRelatedWork is not found in the empty JSON string",
            VersionRelatedWork.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!VersionRelatedWork.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `VersionRelatedWork` properties. JSON: %s",
    ...
    if ((jsonObj.get("relatedWorkId") != null && !jsonObj.get("relatedWorkId").isJsonNull())
        && !jsonObj.get("relatedWorkId").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `relatedWorkId` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("relatedWorkId").toString()));
    }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<VersionRelatedWork> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(VersionRelatedWork.class));
