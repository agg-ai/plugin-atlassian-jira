...
public class FieldDetails {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("clauseNames", "custom", "id", "key", "name", "navigable",
        "orderable", "schema", "scope", "searchable"));
    ...
    if (jsonElement == null) {
      if (!FieldDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(
            String.format(Locale.ROOT, "The required field(s) %s in FieldDetails is not found in the empty JSON string",
                FieldDetails.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!FieldDetails.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `FieldDetails` properties. JSON: %s",
            entry.getKey(), jsonElement.toString()));
      }
    }
    JsonObject jsonObj = jsonElement.getAsJsonObject();
    // ensure the optional json data is an array if present
    if (jsonObj.get("clauseNames") != null && !jsonObj.get("clauseNames").isJsonNull()
        && !jsonObj.get("clauseNames").isJsonArray()) {
      throw new IllegalArgumentException(
          String.format(Locale.ROOT, "Expected the field `clauseNames` to be an array in the JSON string but got `%s`",
              jsonObj.get("clauseNames").toString()));
    }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<FieldDetails> thisAdapter = gson.getDelegateAdapter(this, TypeToken.get(FieldDetails.class));
