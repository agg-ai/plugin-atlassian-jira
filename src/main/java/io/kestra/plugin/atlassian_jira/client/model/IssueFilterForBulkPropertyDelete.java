...
public class IssueFilterForBulkPropertyDelete {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("currentValue", "entityIds"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!IssueFilterForBulkPropertyDelete.openapiRequiredFields.isEmpty()) { // has required fields but JSON element
                                                                               // is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in IssueFilterForBulkPropertyDelete is not found in the empty JSON string",
            IssueFilterForBulkPropertyDelete.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!IssueFilterForBulkPropertyDelete.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `IssueFilterForBulkPropertyDelete` properties. JSON: %s",
            entry.getKey(), jsonElement.toString()));
      }
    }
    JsonObject jsonObj = jsonElement.getAsJsonObject();
    // ensure the optional json data is an array if present
    if (jsonObj.get("entityIds") != null && !jsonObj.get("entityIds").isJsonNull()
        && !jsonObj.get("entityIds").isJsonArray()) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!IssueFilterForBulkPropertyDelete.class.isAssignableFrom(type.getRawType())) {
        return null; // this class only serializes 'IssueFilterForBulkPropertyDelete' and its
                     // subtypes
      }
      final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
      final TypeAdapter<IssueFilterForBulkPropertyDelete> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(IssueFilterForBulkPropertyDelete.class));
