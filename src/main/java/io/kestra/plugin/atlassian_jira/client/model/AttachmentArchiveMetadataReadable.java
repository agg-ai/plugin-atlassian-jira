...
import io.kestra.plugin.atlassian_jira.client.model.AttachmentArchiveItemReadable;
...
public class AttachmentArchiveMetadataReadable {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("entries", "id", "mediaType", "name", "totalEntryCount"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!AttachmentArchiveMetadataReadable.openapiRequiredFields.isEmpty()) { // has required fields but JSON element
                                                                                // is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in AttachmentArchiveMetadataReadable is not found in the empty JSON string",
            AttachmentArchiveMetadataReadable.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!AttachmentArchiveMetadataReadable.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `AttachmentArchiveMetadataReadable` properties. JSON: %s",
      ...
      if (jsonArrayentries != null) {
        ...
        if (!jsonObj.get("entries").isJsonArray()) {
          throw new IllegalArgumentException(
              String.format(Locale.ROOT, "Expected the field `entries` to be an array in the JSON string but got `%s`",
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!AttachmentArchiveMetadataReadable.class.isAssignableFrom(type.getRawType())) {
        return null; // this class only serializes 'AttachmentArchiveMetadataReadable' and its
                     // subtypes
      }
      final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
      final TypeAdapter<AttachmentArchiveMetadataReadable> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(AttachmentArchiveMetadataReadable.class));
