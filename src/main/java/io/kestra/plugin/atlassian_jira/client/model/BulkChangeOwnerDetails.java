...
public class BulkChangeOwnerDetails {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    if (jsonElement == null) {
      if (!BulkChangeOwnerDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The required field(s) %s in BulkChangeOwnerDetails is not found in the empty JSON string",
            BulkChangeOwnerDetails.openapiRequiredFields.toString()));
      }
    }
    ...
    for (Map.Entry<String, JsonElement> entry : entries) {
      if (!BulkChangeOwnerDetails.openapiFields.contains(entry.getKey())) {
        throw new IllegalArgumentException(String.format(Locale.ROOT,
            "The field `%s` in the JSON string is not defined in the `BulkChangeOwnerDetails` properties. JSON: %s",
    ...
    if (!jsonObj.get("newOwner").isJsonPrimitive()) {
      throw new IllegalArgumentException(String.format(Locale.ROOT,
          "Expected the field `newOwner` to be a primitive type in the JSON string but got `%s`",
          jsonObj.get("newOwner").toString()));
    }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      final TypeAdapter<BulkChangeOwnerDetails> thisAdapter = gson.getDelegateAdapter(this,
          TypeToken.get(BulkChangeOwnerDetails.class));
