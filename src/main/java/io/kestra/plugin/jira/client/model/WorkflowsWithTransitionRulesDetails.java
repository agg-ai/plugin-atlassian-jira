...
public class WorkflowsWithTransitionRulesDetails {
   ...
   * @throws IOException if the JSON Element is invalid with respect to WorkflowsWithTransitionRulesDetails
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!WorkflowsWithTransitionRulesDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in WorkflowsWithTransitionRulesDetails is not found in the empty JSON string", WorkflowsWithTransitionRulesDetails.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!WorkflowsWithTransitionRulesDetails.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `WorkflowsWithTransitionRulesDetails` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
