...
import io.kestra.plugin.jira.client.model.JqlQueryToSanitize;
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!JqlQueriesToSanitize.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in JqlQueriesToSanitize is not found in the empty JSON string", JqlQueriesToSanitize.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!JqlQueriesToSanitize.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `JqlQueriesToSanitize` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
