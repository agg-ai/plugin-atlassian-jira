...
public class BulkPermissionGrants {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("globalPermissions", "projectPermissions"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("globalPermissions", "projectPermissions"));
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to BulkPermissionGrants
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BulkPermissionGrants.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in BulkPermissionGrants is not found in the empty JSON string", BulkPermissionGrants.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BulkPermissionGrants.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `BulkPermissionGrants` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("globalPermissions") == null) {
        ...
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `globalPermissions` to be an array in the JSON string but got `%s`", jsonObj.get("globalPermissions").toString()));
      }
      // ensure the json data is an array
      if (!jsonObj.get("projectPermissions").isJsonArray()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `projectPermissions` to be an array in the JSON string but got `%s`", jsonObj.get("projectPermissions").toString()));
      }
      ...
      for (int i = 0; i < jsonArrayprojectPermissions.size(); i++) {
        BulkProjectPermissionGrants.validateJsonElement(jsonArrayprojectPermissions.get(i));
      };
  }
