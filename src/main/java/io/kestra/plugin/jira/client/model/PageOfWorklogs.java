  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PageOfWorklogs.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in PageOfWorklogs is not found in the empty JSON string", PageOfWorklogs.openapiRequiredFields.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // ensure the optional json data is an array if present
      if (jsonObj.get("worklogs") != null && !jsonObj.get("worklogs").isJsonNull() && !jsonObj.get("worklogs").isJsonArray()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `worklogs` to be an array in the JSON string but got `%s`", jsonObj.get("worklogs").toString()));
      }
  }
