...
public class UiModificationIdentifiers {
   ...
   * @throws IOException if the JSON Element is invalid with respect to UiModificationIdentifiers
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!UiModificationIdentifiers.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in UiModificationIdentifiers is not found in the empty JSON string", UiModificationIdentifiers.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!UiModificationIdentifiers.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `UiModificationIdentifiers` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
