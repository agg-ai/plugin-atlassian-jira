...
import io.kestra.plugin.jira.client.model.JiraExpressionEvaluationMetaDataBean;
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!JiraExpressionResult.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in JiraExpressionResult is not found in the empty JSON string", JiraExpressionResult.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!JiraExpressionResult.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `JiraExpressionResult` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
