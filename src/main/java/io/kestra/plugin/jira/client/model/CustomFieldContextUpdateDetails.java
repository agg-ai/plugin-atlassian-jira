...
public class CustomFieldContextUpdateDetails {
   ...
   * @throws IOException if the JSON Element is invalid with respect to CustomFieldContextUpdateDetails
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CustomFieldContextUpdateDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in CustomFieldContextUpdateDetails is not found in the empty JSON string", CustomFieldContextUpdateDetails.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!CustomFieldContextUpdateDetails.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `CustomFieldContextUpdateDetails` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
