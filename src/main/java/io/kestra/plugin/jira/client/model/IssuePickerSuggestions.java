...
import io.kestra.plugin.jira.client.model.IssuePickerSuggestionsIssueType;
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!IssuePickerSuggestions.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in IssuePickerSuggestions is not found in the empty JSON string", IssuePickerSuggestions.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!IssuePickerSuggestions.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `IssuePickerSuggestions` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
