...
public class IssuePickerSuggestions {
  ...
  @JsonCreator
  public IssuePickerSuggestions(
    @JsonProperty(JSON_PROPERTY_SECTIONS) List<IssuePickerSuggestionsIssueType> sections
  ) {
  this();
  ...
  /**
   * Return true if this IssuePickerSuggestions object is equal to o.
   */
  @Override
    ...
    if (getSections() != null) {
      for (int i = 0; i < getSections().size(); i++) {
        if (getSections().get(i) != null) {
          joiner.add(getSections().get(i).toUrlQueryString(String.format(Locale.ROOT, "%ssections%s%s", prefix, suffix,
