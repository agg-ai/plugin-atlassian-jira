...
public class IssueBulkEditPayload {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("editedFieldsInput", "selectedActions", "selectedIssueIdsOrKeys", "sendBulkNotification"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("editedFieldsInput", "selectedActions", "selectedIssueIdsOrKeys"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!IssueBulkEditPayload.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in IssueBulkEditPayload is not found in the empty JSON string", IssueBulkEditPayload.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!IssueBulkEditPayload.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `IssueBulkEditPayload` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("selectedActions") == null) {
        ...
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `selectedActions` to be an array in the JSON string but got `%s`", jsonObj.get("selectedActions").toString()));
      }
      // ensure the required json array is present
