...
public class BulkChangeOwnerDetails {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("autofixName", "newOwner"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("autofixName", "newOwner"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BulkChangeOwnerDetails.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in BulkChangeOwnerDetails is not found in the empty JSON string", BulkChangeOwnerDetails.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BulkChangeOwnerDetails.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `BulkChangeOwnerDetails` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (!jsonObj.get("newOwner").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `newOwner` to be a primitive type in the JSON string but got `%s`", jsonObj.get("newOwner").toString()));
      }
  }
