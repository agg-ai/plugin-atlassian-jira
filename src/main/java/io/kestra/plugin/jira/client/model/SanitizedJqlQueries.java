...
public class SanitizedJqlQueries {
   ...
   * @throws IOException if the JSON Element is invalid with respect to SanitizedJqlQueries
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SanitizedJqlQueries.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in SanitizedJqlQueries is not found in the empty JSON string", SanitizedJqlQueries.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SanitizedJqlQueries.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `SanitizedJqlQueries` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
