...
public class SetDefaultPriorityRequest {
   ...
   * @throws IOException if the JSON Element is invalid with respect to SetDefaultPriorityRequest
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SetDefaultPriorityRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in SetDefaultPriorityRequest is not found in the empty JSON string", SetDefaultPriorityRequest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SetDefaultPriorityRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `SetDefaultPriorityRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
