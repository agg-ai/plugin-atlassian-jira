...
import io.kestra.plugin.jira.client.model.JqlQueryOrderByClauseElement;
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!JqlQueryOrderByClause.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in JqlQueryOrderByClause is not found in the empty JSON string", JqlQueryOrderByClause.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!JqlQueryOrderByClause.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `JqlQueryOrderByClause` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
