...
public class IssueTypeToContextMapping {
   ...
   * @throws IOException if the JSON Element is invalid with respect to IssueTypeToContextMapping
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!IssueTypeToContextMapping.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in IssueTypeToContextMapping is not found in the empty JSON string", IssueTypeToContextMapping.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!IssueTypeToContextMapping.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `IssueTypeToContextMapping` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
