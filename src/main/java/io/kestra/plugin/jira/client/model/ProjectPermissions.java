...
public class ProjectPermissions {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("canEdit"));
   ...
   * @throws IOException if the JSON Element is invalid with respect to ProjectPermissions
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ProjectPermissions.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in ProjectPermissions is not found in the empty JSON string", ProjectPermissions.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ProjectPermissions.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `ProjectPermissions` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
