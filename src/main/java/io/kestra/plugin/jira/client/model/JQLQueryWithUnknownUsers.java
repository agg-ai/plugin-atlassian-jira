...
public class JQLQueryWithUnknownUsers {
   ...
   * @throws IOException if the JSON Element is invalid with respect to JQLQueryWithUnknownUsers
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!JQLQueryWithUnknownUsers.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in JQLQueryWithUnknownUsers is not found in the empty JSON string", JQLQueryWithUnknownUsers.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!JQLQueryWithUnknownUsers.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `JQLQueryWithUnknownUsers` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("convertedQuery") != null && !jsonObj.get("convertedQuery").isJsonNull()) && !jsonObj.get("convertedQuery").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `convertedQuery` to be a primitive type in the JSON string but got `%s`", jsonObj.get("convertedQuery").toString()));
      }
      if ((jsonObj.get("originalQuery") != null && !jsonObj.get("originalQuery").isJsonNull()) && !jsonObj.get("originalQuery").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `originalQuery` to be a primitive type in the JSON string but got `%s`", jsonObj.get("originalQuery").toString()));
      }
  }
