...
public class PageBeanContextForProjectAndIssueType {
   ...
   * @throws IOException if the JSON Element is invalid with respect to PageBeanContextForProjectAndIssueType
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PageBeanContextForProjectAndIssueType.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in PageBeanContextForProjectAndIssueType is not found in the empty JSON string", PageBeanContextForProjectAndIssueType.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PageBeanContextForProjectAndIssueType.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `PageBeanContextForProjectAndIssueType` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
