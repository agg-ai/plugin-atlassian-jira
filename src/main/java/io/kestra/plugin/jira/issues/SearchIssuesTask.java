...
import io.kestra.plugin.jira.AbstractTask;
import io.kestra.plugin.jira.client.api.IssueSearchApi;
import io.kestra.plugin.jira.client.model.SearchAndReconcileResults;
import io.kestra.plugin.jira.helpers.PropertyHelper;
...
public class SearchIssuesTask extends AbstractTask implements RunnableTask<SearchAndReconcileResults> {
    @Schema(title = "JQL query to search for issues", description = "JQL query to search for issues. Defaults to 'project = PROJ AND status = Open'.")
    ...
    public SearchAndReconcileResults run(RunContext runContext) throws Exception {
        ...
        var renderedFields = PropertyHelper.safeRenderList(runContext, fields, null, String.class);
